...
const initializers = {
  ...
  eventstorming: async () => {
    const { createEventStormingServices } = await import('./language/eventstorming/index.js');
    const parser = createEventStormingServices().EventStorming.parser.LangiumParser;
    parsers.eventstorming = parser;
  },
} as const;
...
export async function parse(diagramType: 'eventstorming', text: string): Promise<EventStorming>;
